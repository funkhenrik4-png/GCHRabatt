...
function book() {
  ...
  fetch(webAppUrl, {
    method: "POST",
    body: JSON.stringify({ date, start, end, simulator, numPlayers, playerNumbers })
  }).then(res => res.json())
    .then(data => {
...
function loadAdminData() {
  fetch(webAppUrl)
    ...
    .then(data => {
      ...
      data.players.forEach(p => { html += `<tr><td>${p.DGV}</td><td>${p.AK}</td></tr>`; });
      html += "</table>";
      playerDiv.innerHTML = html;

      // Buchungen
      const statsDiv = document.getElementById("stats");
      let html2 = "<table><tr><th>Datum</th><th>Start</th><th>Ende</th><th>AK</th><th>Simulator</th><th>Dauer</th><th>Löschen</th></tr>";
      data.bookings.forEach(b => {
        html2 += `<tr>
          ...
          <td>${b.Simulator}</td>
          <td>${b.Dauer}</td>
          <td><button onclick="deleteBooking('${b.id}')">Löschen</button></td>
        </tr>`;
      });
      html2 += "</table>";
...
function deleteBooking(bookingId) {
  fetch(`${webAppUrl}?action=delete&bookingId=${bookingId}`)
    .then(res => res.json())
    .then(data => {
      if(data.status === "ok") loadAdminData();
    });
}
